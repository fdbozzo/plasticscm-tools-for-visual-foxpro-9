*------------------------------------------------------------------------------
*-- FOXPROMERGE.PRG		- Visual FoxPro 9.0 MERGE TOOL para PlasticSCM
*-- Fernando D. Bozzo	- 20/12/2013
*------------------------------------------------------------------------------
LPARAMETERS tcSourcePath, tcBasePath, tcDestinationPath, tcSourceSymbolic, tcBaseSymbolic, tcDestinationSymbolic, tcOutputPath

#DEFINE CR_LF	CHR(13) + CHR(10)

LOCAL loEx AS EXCEPTION, loMerge AS CL_MERGE OF 'FOXPROMERGE.PRG'
loMerge = CREATEOBJECT('CL_MERGE')
loMerge.lDebug	= .T.
loMerge.PROCESS( @loEx, tcSourcePath, tcBasePath, tcDestinationPath, tcSourceSymbolic, tcBaseSymbolic, tcDestinationSymbolic, tcOutputPath )
...
DEFINE CLASS CL_MERGE AS CUSTOM
	_MEMBERDATA	= [<VFPData>] ;
	...
	PROCEDURE PROCESS
		LPARAMETERS toEx AS EXCEPTION, tcSourcePath, tcBasePath, tcDestinationPath, tcSourceSymbolic, tcBaseSymbolic, tcDestinationSymbolic, tcOutputPath
		...
		TRY
			LOCAL lnExitCode AS INTEGER ;
				...
				, lcMenError, lcTextMergeResult ;
				, loShell AS WScript.SHELL

			WITH THIS AS CL_MERGE OF 'FOXPROMERGE.PRG'
				...
				loShell				= .oShell
				.cSys16				= SYS(16)
				.cEXEPath			= JUSTPATH( SUBSTR( .cSys16, AT( GETWORDNUM(.cSys16, 2), .cSys16 ) + LEN(.cSys16) + 1 ) )
			...
			ENDWITH && THIS AS CL_MERGE OF 'FOXPROMERGE.PRG'
	...
	FUNCTION SourceProcess
		...
		#IF .F.
			LOCAL THIS AS CL_MERGE OF 'FOXPROMERGE.PRG'
		#ENDIF
	...
	FUNCTION writeLog
		...
		IF THIS.lDebug
			TRY
				tcText	= EVL(tcText,'')
				STRTOFILE( TRANSFORM(tcText) + CR_LF, FORCEPATH( 'foxpromerge.log', GETENV("TEMP") ), 1 )
			CATCH
			ENDTRY
		ENDIF
